...
3. Custom Start Command:
   ...
   ```bash
   sh -lc "rm -f /nakama/data/*.local.yml || true; \
   /nakama/nakama migrate up --database.address='${DATABASE_URL}' || true; \
   /nakama/nakama --name nakama --database.address='${DATABASE_URL}' \
   --logger.level DEBUG --runtime.path /nakama/modules"
   ```
...
6. Click **Play Again** to reset the match.

---

This completes the full assignment deliverables for Lila Games.
